...
const Tag = ({children,className,color}:{
  ...
  color?:TagColor
    ...
    const colorMap:Record<string,string> = {
        ...
        error: 'text-error border-error bg-red-400/10',
  ...
  return (
    <div className={`inline-block px-2 py-[2px] text-[8px] md:text-[11px] text-black border border-black rounded-full ${className} ${colorMap[color||'default']}`}>
      {children}
    </div>
  );
};
